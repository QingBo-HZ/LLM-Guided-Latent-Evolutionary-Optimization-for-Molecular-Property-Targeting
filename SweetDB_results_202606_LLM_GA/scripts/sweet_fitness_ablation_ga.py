...
class SweetGatedEvaluator:
    def __init__(
        ...
        self.docking_predictor_dir = docking_predictor_dir

        self._load_classifier()
        self._load_regressor()
        self._load_ood()
        self._load_docking_regressor()
    ...
    def _load_docking_regressor(self):
        self.docking_regressor_members = []
        if self.docking_predictor_dir is None:
            if self.fitness_mode in {
                "docking_surrogate",
                "direct_combo",
                "gated_docking",
                "gated_docking_combo",
            }:
                raise ValueError("docking_predictor_dir is required for docking_surrogate fitness")
            return

        ensemble_path = os.path.join(
            self.docking_predictor_dir, "latent_docking_ensemble_summary.json"
        )
        if not os.path.exists(ensemble_path):
            raise FileNotFoundError(f"Docking ensemble summary not found: {ensemble_path}")

        with open(ensemble_path, "r", encoding="utf-8") as handle:
            self.docking_regressor_summary = json.load(handle)

        for member in self.docking_regressor_summary.get("models", []):
            member_dir = os.path.join(self.docking_predictor_dir, member["dir"])
            bundle = safe_torch_load(
                os.path.join(member_dir, "latent_docking_regressor_bundle.pt")
            )
            model = LatentRegressor(
                latent_dim=self.latent_dim,
                hidden_dim=int(bundle.get("hidden_dim", 128)),
            ...
            model.load_state_dict(bundle["state_dict"])
            model.eval()
            self.docking_regressor_members.append({
                "fold": int(member.get("fold", len(self.docking_regressor_members))),
                "model": model,
                "scaler_z": joblib.load(
                    os.path.join(member_dir, "latent_docking_scaler_z.pkl")
                ),
                "scaler_y": joblib.load(
                    os.path.join(member_dir, "latent_docking_scaler_y.pkl")
                ),
            })

        if not self.docking_regressor_members:
            raise ValueError(f"No docking regressor members found in {ensemble_path}")
        print(
            f"[Evaluator] Loaded docking surrogate ensemble: "
            f"{len(self.docking_regressor_members)} members"
        )
    ...
    def evaluate(self, z):
        ...
        docking_score = np.full(len(z), np.nan, dtype=np.float32)
        docking_score_std = np.full(len(z), np.nan, dtype=np.float32)
        if self.docking_regressor_members:
            docking_member_preds = []
            with torch.no_grad():
                for member in self.docking_regressor_members:
                    z_docking = member["scaler_z"].transform(z)
                    x_docking = torch.tensor(
                        z_docking, dtype=torch.float32, device=self.device
                    )
                    pred_scaled = (
                        member["model"](x_docking).detach().cpu().numpy().astype(np.float32)
                    )
                    pred_raw = member["scaler_y"].inverse_transform(
                        pred_scaled.reshape(-1, 1)
                    ).reshape(-1).astype(np.float32)
                    docking_member_preds.append(pred_raw)
            docking_member_preds = np.stack(docking_member_preds, axis=0)
            docking_score = docking_member_preds.mean(axis=0).astype(np.float32)
            docking_score_std = docking_member_preds.std(axis=0).astype(np.float32)
        ...
        if self.objective == "constrained_sweetness":
            ...
            gated_base_score = p_sweet * pred_logsw_clipped - self.lambda_ood * d_ood_norm

        if self.fitness_mode == "direct_regressor":
            score_ga = pred_logsw
        elif self.fitness_mode == "docking_surrogate":
            if not self.docking_regressor_members:
                raise RuntimeError("Docking surrogate was not loaded")
            score_ga = docking_score
        elif self.fitness_mode == "direct_combo":
            if not self.docking_regressor_members:
                raise RuntimeError("Docking surrogate was not loaded")
            score_ga = 0.50 * self._rank01(pred_logsw) + 0.50 * self._rank01(docking_score)
        elif self.fitness_mode == "gated_docking":
            if not self.docking_regressor_members:
                raise RuntimeError("Docking surrogate was not loaded")
            ood_threshold = self.ood_p95 if self.ood_threshold is None else self.ood_threshold
            ood_penalty = np.maximum(0.0, d_ood - ood_threshold) / max(ood_threshold, 1e-8)
        ...
        elif self.fitness_mode == "gated_docking_combo":
            if not self.docking_regressor_members:
                raise RuntimeError("Docking surrogate was not loaded")
            score_ga = (
                0.70 * self._rank01(gated_base_score)
                + 0.30 * self._rank01(docking_score)
            )
        elif self.fitness_mode == "gated":
            score_ga = gated_base_score
        else:
        ...
        return {
            ...
            "docking_score": docking_score.astype(np.float32),
            "docking_score_std": docking_score_std.astype(np.float32),
            "gated_base_score": gated_base_score.astype(np.float32),
            "gate_constraint_violation": (
                np.maximum(0.0, self.p_sweet_threshold - p_sweet) / max(self.p_sweet_threshold, 1e-8)
                + np.maximum(0.0, self.logsw_success_threshold - pred_logsw) / max(abs(self.logsw_success_threshold), 1e-8)
                + np.maximum(
                    0.0,
                    d_ood - (self.ood_p95 if self.ood_threshold is None else self.ood_threshold),
                ) / max((self.ood_p95 if self.ood_threshold is None else self.ood_threshold), 1e-8)
            ).astype(np.float32),
...
def final_rescore_candidates(
    ...
    for _, row in df.iterrows():
        ...
        docking_score_re = np.nan
        docking_score_std_re = np.nan
        ...
        if valid and desc.get("canonical_smiles", None) is not None:
            ...
            if z_re is not None:
                ...
                docking_score_re = float(m_re["docking_score"][0])
                docking_score_std_re = float(m_re["docking_score_std"][0])
        ...
        rec.update({
            ...
            "docking_score_reencoded": docking_score_re,
            "docking_score_std_reencoded": docking_score_std_re,
...
def main():
    ...
    parser.add_argument("--docking_predictor_dir", default=None)
    parser.add_argument(
        "--fitness_mode",
        choices=[
            "direct_regressor",
    ...
    evaluator = SweetGatedEvaluator(
        ...
        docking_predictor_dir=args.docking_predictor_dir,
    )
    ...
    for gen in range(args.n_gen):
        ...
        if args.save_population_history:
            ...
            pd.DataFrame({
                ...
                "d_ood": metrics["d_ood"],
            "docking_score": metrics["docking_score"],
            "docking_score_std": metrics["docking_score_std"],
            "gated_base_score": metrics["gated_base_score"],
        }).to_csv(
        ...
        if best_score > best_score_so_far:
            ...
            best_record_so_far = {
                ...
                "docking_score": float(docking_score[best_idx]),
                "docking_score_std": float(docking_score_std[best_idx]),
                "gated_base_score": float(metrics["gated_base_score"][best_idx]),
            }
            no_improve = 0
        else:
            no_improve += 1
        ...
        progress_records.append({
            ...
            "mean_docking_score": float(np.mean(docking_score)),
            "best_docking_score": float(docking_score[best_idx]),
            "mean_gated_base_score": float(np.mean(metrics["gated_base_score"])),
            "best_gated_base_score": float(metrics["gated_base_score"][best_idx]),
        ...
        for rank, idx in enumerate(archive_idx):
            row = {
                ...
                "docking_score": float(docking_score[idx]),
                "docking_score_std": float(docking_score_std[idx]),
                "gated_base_score": float(metrics["gated_base_score"][idx]),
    ...
    for i in range(len(population)):
        base_rows.append({
            ...
            "docking_score": float(final_metrics["docking_score"][i]),
            "docking_score_std": float(final_metrics["docking_score_std"][i]),
            "gated_base_score": float(final_metrics["gated_base_score"][i]),
        })
    ...
    summary = {
        ...
        "ga_fitness": (
            "score = pred_logSw"
            if args.fitness_mode == "direct_regressor"
            else (
                "score = latent docking surrogate prediction"
                if args.fitness_mode == "docking_surrogate"
                else (
                    "score = 0.50*rank(pred_logSw) + 0.50*rank(docking)"
                    if args.fitness_mode == "direct_combo"
                    else (
                        "score = docking, after hard gates on P_sweet, logSw, and OOD"
                        if args.fitness_mode == "gated_docking"
                        else (
                            "score = 0.70*rank(gated_sweetness) + 0.30*rank(docking)"
                            if args.fitness_mode == "gated_docking_combo"
                            else (
                                "score = pred_logSw + 0.50*P_sweet - "
                                "0.80*max(0, p_threshold-P_sweet) - "
                                "0.50*max(0, D_OOD-OOD_p95)/OOD_p95"
                                if args.objective == "constrained_sweetness"
                                else (
                                    "score_ga = P_sweet * min(pred_logSw, logsw_score_cap) "
                                    "- lambda_ood * D_OOD_norm"
                                )
                            )
                        )
                    )
        ...
        "mean_docking_score_reencoded": float(
            np.nanmean(final_corrected_df["docking_score_reencoded"].values)
        ),
