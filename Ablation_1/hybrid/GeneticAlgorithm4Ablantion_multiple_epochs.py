...
# 经过多轮优化的 LLM latent 开关，分别对应不同 epoch 的版本
#LLM_LATENT_PATH = "/root/autodl-tmp/sweeteners_evolve/Ablation_1/llm/latent_epoch_1/llm_init_latent.npy"
#LLM_LATENT_PATH = "/root/autodl-tmp/sweeteners_evolve/Ablation_1/llm/latent_epoch_10/llm_init_latent.npy"
#LLM_LATENT_PATH = "/root/autodl-tmp/sweeteners_evolve/Ablation_1/llm/latent_epoch_20/llm_init_latent.npy"
#LLM_LATENT_PATH = "/root/autodl-tmp/sweeteners_evolve/Ablation_1/llm/latent_epoch_40/llm_init_latent.npy"
LLM_LATENT_PATH = "/root/autodl-tmp/sweeteners_evolve/Ablation_1/llm/latent_epoch_80/llm_init_latent.npy"
#LLM_LATENT_PATH = "/root/autodl-tmp/sweeteners_evolve/Ablation_1/llm/latent_epoch_100/llm_init_latent.npy"
...
def build_immigrant_pool(init_mode, llm_latent_path, psvae_latent_path, hybrid_latent_path):
    """Build a fixed external latent pool used to re-inject diversity each generation."""
    pools = []

    def maybe_load(path, name):
        if path is None or not os.path.exists(path):
            return None
        arr = np.load(path).astype(np.float32)
        if arr.ndim != 2 or arr.shape[1] != latent_dim:
            print(f"[WARN] skip {name} immigrant pool due to bad shape: {arr.shape}")
            return None
        return arr

    if init_mode == "llm":
        arr = maybe_load(llm_latent_path, "llm")
        if arr is not None:
            pools.append(arr)
    elif init_mode == "psvae":
        arr = maybe_load(psvae_latent_path, "psvae")
        if arr is not None:
            pools.append(arr)
    elif init_mode == "hybrid":
        for path, name in [
            (hybrid_latent_path, "hybrid"),
            (llm_latent_path, "llm"),
            (psvae_latent_path, "psvae"),
        ]:
            arr = maybe_load(path, name)
            if arr is not None:
                pools.append(arr)
    elif init_mode == "train_random":
        pools.append(latent_train)

    if not pools:
        pools.append(latent_train)

    pool = np.concatenate(pools, axis=0).astype(np.float32)
    print(f"[INFO] immigrant pool shape: {pool.shape}")
    return pool


def append_archive_entries(archive, population, pred_all, pred_gap, generation, topk):
    """Keep low-gap latent candidates for final unique-SMILES selection."""
    if topk <= 0:
        return
    topk = min(int(topk), len(population))
    for rank, idx in enumerate(np.argsort(pred_gap)[:topk]):
        archive.append({
            "generation": int(generation),
            "rank_in_generation": int(rank),
            "latent": population[idx].copy().astype(np.float32),
            "pred": pred_all[idx].copy().astype(np.float32),
...
def select_unique_decoded_archive(
    archive,
    property_names,
    target_size,
    max_decode,
    ...
    """Decode archive by predicted gap and keep the best unique canonical SMILES."""
    sorted_archive = sorted(archive, key=lambda item: item["gap"])
    max_decode = min(int(max_decode), len(sorted_archive))
    ...
    for archive_rank, item in enumerate(sorted_archive[:max_decode]):
        smi = latent_to_smiles(item["latent"], temperature=temperature)
        decode_attempts += 1
        if smi is None:
            continue

        key = canonicalize_smiles(smi)
        if key is None:
            continue
        ...
        row = {
            "idx": len(rows),
            "smiles": key,
            "pred_gap": float(pred[predictor.gap_idx]),
            "source_generation": item["generation"],
            "rank_in_generation": item["rank_in_generation"],
            "archive_rank": archive_rank,
        }
        for j, pname in enumerate(property_names):
            row[pname] = float(pred[j])
    ...
    if preds:
        ...
        latent_arr = np.stack(latents, axis=0).astype(np.float32)
    else:
        pred_arr = np.empty((0, len(property_names)), dtype=np.float32)
        latent_arr = np.empty((0, latent_dim), dtype=np.float32)

    stats = {
        "archive_size": int(len(sorted_archive)),
        "archive_decode_attempts": int(decode_attempts),
        "archive_decode_success": int(decode_success),
        "unique_smiles_selected": int(len(rows)),
        "archive_decode_success_rate": float(decode_success / decode_attempts) if decode_attempts else 0.0,
    }
    return rows, latent_arr, pred_arr, stats
...
def main():
    ...
    parser.add_argument("--random_immigrant_frac", type=float, default=0.05,
                        help="每代用外部 seed pool 随机替换的比例，用于防止种群坍缩；设为 0 可关闭")
    parser.add_argument("--archive_topk_per_gen", type=int, default=10,
                        help="每代保存多少个低 gap latent 到历史候选池，用于最终 unique-SMILES 选择")
    parser.add_argument("--max_archive_decode", type=int, default=3000,
                        help="最终从历史候选池中最多解码多少个 latent 来筛选 unique SMILES")
    parser.add_argument("--final_decode_temperature", type=float, default=0.8,
                        help="最终解码历史候选池时使用的 decoder temperature")
    ...
    immigrant_pool = build_immigrant_pool(
        init_mode=args.init_mode,
        llm_latent_path=args.llm_latent_path,
        psvae_latent_path=args.psvae_latent_path,
        hybrid_latent_path=args.hybrid_latent_path,
    ...
    for gen in range(args.n_gen):
        ...
        append_archive_entries(
            ...
            pred_gap,
            generation=gen,
            topk=args.archive_topk_per_gen,
        )
        ...
        n_immigrants = int(round(args.pop_size * args.random_immigrant_frac))
        n_immigrants = min(max(n_immigrants, 0), max(args.pop_size - args.elite_size, 0))
        if n_immigrants > 0:
            immigrants = sample_population_from_pool(immigrant_pool, n_immigrants)
            new_population[-n_immigrants:] = list(immigrants)
    ...
    # 最终评估：先保留 raw last generation，再从历史 archive 里选 unique SMILES
    print("对最终种群进行预测...")
    raw_population = population.copy()
    raw_pred = predictor.predict_array(raw_population)
    raw_gap = raw_pred[:, predictor.gap_idx]
    append_archive_entries(
        candidate_archive,
        raw_population,
        raw_pred,
        raw_gap,
        generation=len(avg_gap_history),
        topk=len(raw_population),
    )

    property_names = predictor.property_names

    print("解码 raw 最后一代种群为 SMILES（用于诊断坍缩，不作为最终 unique 输出）...")
    raw_decoded_smiles = []
    raw_decode_success = 0
    for i in range(len(raw_population)):
        smi = latent_to_smiles(raw_population[i], temperature=args.final_decode_temperature)
        raw_decoded_smiles.append(smi)
        if smi is not None:
            raw_decode_success += 1

    raw_rows = []
    for i in range(len(raw_population)):
        row = {
            "idx": i,
            "smiles": raw_decoded_smiles[i],
            "pred_gap": float(raw_gap[i]),
        }
        for j, pname in enumerate(property_names):
            row[pname] = float(raw_pred[i, j])
        raw_rows.append(row)

    raw_csv_path = os.path.join(out_dir, f"final_population_raw_{args.version}.csv")
    pd.DataFrame(raw_rows).to_csv(raw_csv_path, index=False)
    np.save(os.path.join(out_dir, "final_population_raw_latent.npy"), raw_population)
    np.save(os.path.join(out_dir, "final_population_raw_pred.npy"), raw_pred)
    np.save(os.path.join(out_dir, "final_population_raw_gap.npy"), raw_gap)
    print(f"[INFO] raw final population saved to: {raw_csv_path}")
    print(f"[INFO] raw decode success: {raw_decode_success}/{len(raw_population)}")

    print("从历史 archive 中按 predicted gap 排序并筛选 unique SMILES...")
    rows, final_population, final_pred, archive_stats = select_unique_decoded_archive(
        archive=candidate_archive,
        property_names=property_names,
        target_size=args.pop_size,
        max_decode=args.max_archive_decode,
        temperature=args.final_decode_temperature,
    )

    if len(rows) == 0:
        raise RuntimeError("历史候选池没有解码出任何有效 unique SMILES。")
    if len(rows) < args.pop_size:
        print(
            f"[WARN] unique final population has only {len(rows)}/{args.pop_size} molecules. "
            f"Try increasing --max_archive_decode or --archive_topk_per_gen."
        )

    final_gap = final_pred[:, predictor.gap_idx]
    decoded_smiles = [row["smiles"] for row in rows]
    valid_smiles = decoded_smiles
    diversity = compute_diversity(valid_smiles) if len(valid_smiles) > 1 else 0.0
    decode_success = len(rows)
    decode_rate = 1.0 if len(rows) > 0 else 0.0
    ...
    print(f"最终 unique 种群已保存到: {final_csv_path}")
    print(f"[INFO] unique smiles selected: {len(rows)}/{args.pop_size}")

    # 保存最终 unique latent 与预测结果
    np.save(os.path.join(out_dir, "final_population_latent.npy"), final_population)
    ...
    summary = {
        ...
        "archive_topk_per_gen": args.archive_topk_per_gen,
        "max_archive_decode": args.max_archive_decode,
        "final_decode_temperature": args.final_decode_temperature,
        ...
        "unique_smiles_final": int(len(set(decoded_smiles))),
        "raw_unique_smiles_final": int(len(set([s for s in raw_decoded_smiles if s is not None]))),
        "raw_diversity_final": compute_diversity([s for s in raw_decoded_smiles if s is not None]) if raw_decode_success > 1 else 0.0,
        "raw_validity_final": float(raw_decode_success / len(raw_decoded_smiles)) if len(raw_decoded_smiles) > 0 else 0.0,
        **archive_stats,
        ...
        "final_population_raw_csv": raw_csv_path,
