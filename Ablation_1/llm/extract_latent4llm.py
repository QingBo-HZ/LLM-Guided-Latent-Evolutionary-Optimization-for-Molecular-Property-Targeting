...
# 4. 去重（按 smiles 文本去重）
# ============================================================
def deduplicate_smiles(smiles_records):
    seen = set()
    unique_records = []
    dup_count = 0
    ...
    print(f"[DEBUG] deduplicated smiles: {len(smiles_records)} -> {len(unique_records)}, duplicates={dup_count}", flush=True)
...
# 5. 提取 latent
# ============================================================
def extract_latent(smiles_records, model):
    x_list = []
    meta_rows = []
...
# 6. 保存输出
# ============================================================
def save_outputs(X, meta_rows, out_dir):
    ...
    with open(meta_path, "w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["row_idx", "smiles", "source_file", "source_type"])
        writer.writeheader()
    ...
    return x_path, smiles_path, meta_path
...
def main(args):
    ...
    smiles_records = deduplicate_smiles(smiles_records)

    X, meta_rows = extract_latent(smiles_records, model)

    save_outputs(X, meta_rows, args.out_dir)
