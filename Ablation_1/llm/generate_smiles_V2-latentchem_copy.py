...
# 将 LatentChem 代码目录放在最前面
latentchem_path = '/root/autodl-tmp/lc/LatentChem/code_train_sft'
if latentchem_path not in sys.path:
    sys.path.insert(0, latentchem_path)
...
class LatentChemGenerator:
    def __init__(self, ckpt_dir, device="cuda", torch_dtype=torch.bfloat16):
        self.device = device
        ...
        mol_config = {
            ...
            'smi_ted_folder': ModelConfig.DEFAULT_SMI_TED_FOLDER,
            'smi_ted_ckpt': ModelConfig.DEFAULT_SMI_TED_CKPT,
        }

        # 2. 初始化模型（与 inference.py 中一致）
        self.model = Qwen3MoleculeLLM(
        ...
        # 3. 加载 checkpoint
        lora_path = os.path.join(ckpt_dir, "lora_weights")
        projector_path = os.path.join(ckpt_dir, "mm_projector.pt")
        self.model = load_trained_components_stage3(
            self.model,
            lora_weights_path=lora_path,
            mm_projector_path=projector_path
        )
        self.model.to(device)
        self.model.eval()

        self.tokenizer = self.model.tokenizer
        self.tokenizer.padding_side = "left"   # 推理时左对齐

    def generate_candidates(self, prompt: str, n: int = 1, temperature: float = 0.8, max_new_tokens: int = 512) -> str:
        import re, json

        # 1. 构造与训练一致的输入（末尾加 <answer> 提示模型开始输出）
        # formatted_prompt = f"{prompt}\nOutput: <answer>"

        inputs = self.tokenizer(prompt, return_tensors="pt", truncation=True, max_length=1024).to(self.device)
        batch_smiles = [[] for _ in range(inputs.input_ids.size(0))]
        ...
        with torch.no_grad():
            generated_ids = self.model.generate(
                ...
                do_sample=True,
                top_p=0.9,
                num_return_sequences=1,          # 关键：一次生成一个完整回答
            )

        full_output = self.tokenizer.decode(generated_ids[0], skip_special_tokens=True)
        print("full_output: ",full_output)
        # 2. 提取 <answer> 标签内的内容
        match = re.search(r'<answer>(.*?)</answer>', full_output, re.DOTALL)
        if not match:
            # 降级：尝试找任何看起来像 SMILES 的行（无空格、不包含 step 等）
            lines = [line.strip() for line in full_output.split('\n') if line.strip()]
            smiles_list = [line for line in lines if not line.startswith(('Step', 'step', '{', '['))]
        else:
        ...
        # 4. 返回 JSON 字符串（与 parse_generation_output 兼容）
        return json.dumps({"smiles": unique}, ensure_ascii=False)

_generator = None

def get_generator(ckpt_dir="/root/autodl-tmp/lc/LatentChem/checkpoints", device="cuda"):
    global _generator
    if _generator is None:
        _generator = LatentChemGenerator(ckpt_dir, device)
    return _generator
...
You are an expert chemist. Given the molecular design task, constraints, and design rules, your job is to propose chemically plausible molecules that satisfy all requirements.
...
- Propose exactly {n_candidates} unique seed molecules.
...
"SMILES_2",
...
"SMILES_{n_candidates}"
</answer>

Do NOT include any additional text, explanations, step-by-step reasoning, markdown formatting, or JSON. Output only the <answer> block.
""".strip()
...
- Only return smiles the last step generated.
- No markdown.
- No commentary.
- No explanation.
- JSON only.
...
def parse_generation_output(raw_text: str) -> Dict:
    # print("\n\nparse_generation_output(raw_text::   ",raw_text)
    # obj = raw_text
...
def run_weak_llema_generation(
    ...
    parse_err_dir = Path("./smiles_generated_latentchem")
    ...
    for gen_idx in range(generations):
        ...
        generator = get_generator(
            ckpt_dir="/root/autodl-tmp/lc/LatentChem/checkpoints/124-taskthinker-bioupdater/stage4",  # 请确认路径
            device="cuda"
        )
        raw_text = generator.generate_candidates(prompt, n=n_candidates_per_gen, temperature=0.8)
        # raw_text_2 = call_llm_json(
        #     prompt=prompt,
        ...
        for raw_smiles in parsed["smiles"]:
            raw_smiles = clean_smiles(raw_smiles)
            if not raw_smiles:
                rejected_stats["empty_after_clean"] = rejected_stats.get("empty_after_clean", 0) + 1
                continue
            
            # 过滤非 SMILES 字符串（关键）
            if not re.fullmatch(r"[A-Za-z0-9@+\-\[\]\(\)=#$\\/%.]+", raw_smiles):
                rejected_stats["invalid_chars"] = rejected_stats.get("invalid_chars", 0) + 1
                continue

            print("CLEAN_SMILES:", repr(raw_smiles))  # debug 用，跑通后可删
...
def main():
    ...
    parser.add_argument("--out_dir", type=str, default="./smiles_generated_latentchem")
