...
SYSTEM_PROMPT = """
You are a rigorous molecular design assistant for benchmarked iterative generation.
You must follow the task card exactly.
You must respect constraints, design rules, and memory.
...
{{
  ...
  "candidate_sources": [
    {{"smiles": "SMILES_1", "source": "initial_exploration"}},
    {{"smiles": "SMILES_2", "source": "initial_exploration"}}
  ],
...
- "candidate_sources" must contain the same molecules as in "smiles".
- "source" for generation 0 must be "initial_exploration".
- No markdown.
- No commentary.
- JSON only.
""".strip()
...
- You may propose candidates inspired by:
  1. mutation_of_success
  2. recombination_of_success
  3. guided_exploration
...
{{
  ...
  "applied_rules": ["rule 1", "rule 2"],
  "candidate_sources": [
    {{"smiles": "SMILES_1", "source": "mutation_of_success"}},
    {{"smiles": "SMILES_2", "source": "recombination_of_success"}},
    {{"smiles": "SMILES_3", "source": "guided_exploration"}}
  ],
...
Strict requirements:
- Return exactly {n_candidates} unique SMILES in "smiles".
- Every SMILES in "smiles" must appear in "candidate_sources".
- source must be one of:
  "mutation_of_success", "recombination_of_success", "guided_exploration"
- No markdown.
- No commentary.
- JSON only.
""".strip()
...
def run_weak_llema_generation(
    ...
    all_entries = []            # 全部候选的详细记录
    accepted_global = []        # 全局已接受 canonical smiles
    accepted_set = set()
    generation_logs = []
